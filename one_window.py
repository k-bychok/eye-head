...
def eye_tracker(nh, nv,
                ic_h, ic_v, oc_h, oc_v,        # ←-- НОВОЕ!
                side):
    """Мини-виджет для глаза: pupil + реальные уголки"""
    ...
    cv2.circle(img, (ix, iy), 4, (255, 0, 0), -1)  # inner-corner
    cv2.circle(img, (ox, oy), 4, (255, 0, 0), -1)  # outer-corner
...
def update_frame():
    ...
    if res.multi_face_landmarks:
        ...
        # --- вертикаль (та же v_norm, что и для зрачка)
        right_ic_v = v_norm(159, 145, 133)  # индексы верх/низ века + уголок
