"""
Head/Eye-tracker с масштабируемым UI
Автор: …
...
# -------------------------------------------------------
# --- геометрия второго окна ---------------------------------------------
SECOND_WINDOW_SIZE = "2700x1500"          # ← меняйте как нужно
...
kx, ky = WIN_W / BASE_W, WIN_H / BASE_H    # анизотропное масштабирование
k      = min(kx, ky)                       # изотропное (для кругов и т.п.)
...
PLATE_SIZE      = int(30 * k)
ARROW_LEN       = int(60 * k)              # длина стрелки yaw
...
# -------------------------------------------------------
#          ЗАГРУЗКА КООРДИНАТ ТАРЕЛОК-ЦЕЛЕЙ
# -------------------------------------------------------
def load_squares():
    try:
...
# -------------------------------------------------------
#        ВСПОМОГАТЕЛЬНАЯ МАТЕМАТИКА (голова / глаза)
# -------------------------------------------------------
...
def update_frame():
    ...
    if res.multi_face_landmarks:
        ...
        yaw_history.append(yaw); yaw_history[:] = yaw_history[-max_history:]
